...
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await generateAgedReceivablesReport(filters);

    if (!report) {
      return NextResponse.json(
        { success: false, error: "Failed to generate aged receivables report" },
        { status: 500 }
      );
    }
    ...
    // Handle CSV export
    if (format === "csv") {
      const csv = generateAgedReceivablesCSV(report);
      return new NextResponse(csv, {
        headers: {
          "Content-Type": "text/csv",
          "Content-Disposition": `attachment; filename="aged-receivables-${new Date().toISOString().split('T')[0]}.csv"`,
        },
      });
    }
