...
export async function POST(request: NextRequest) {
  try {
    ...
    // Log audit trail
    for (const r of res) {
      await logAuditAction('suspend', 'tenant', r.id.toString(), { count: res.length }, r.slug, request.ip);
    }
    ...
    console.error('Error bulk suspending tenants:', error);
    return NextResponse.json({ error: 'Failed to bulk suspend tenants' }, { status: 500 });
  }
}
