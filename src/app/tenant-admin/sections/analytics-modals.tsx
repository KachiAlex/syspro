      ...
      <div className="w-full max-w-md rounded-xl bg-white shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 px-6 py-6">
          <div>
            ...
            <p className="text-sm font-semibold text-slate-900 mt-1 capitalize">{report.type}</p>
          </div>
          ...
          {report.rows && (
            <div className="rounded-lg bg-blue-50 p-3">
              <p className="text-xs font-medium text-blue-900">Report includes {report.rows} rows of data</p>
            </div>
          )}
        ...
        <div className="border-t border-slate-200 px-6 py-4 flex gap-3 sticky bottom-0 bg-white">
          {onEdit && (
            <button
              onClick={onEdit}
              className="flex-1 rounded-lg bg-slate-100 px-4 py-2 text-sm font-medium text-slate-900 hover:bg-slate-200"
            >
              Edit
            </button>
          )}
          <button
            onClick={onClose}
            className={`${onEdit ? "flex-1" : "w-full"} rounded-lg bg-slate-100 px-4 py-2 text-sm font-medium text-slate-900 hover:bg-slate-200`}
          >
            Close
          </button>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-xl bg-white shadow-xl">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <h2 className="text-lg font-semibold text-slate-900">Edit Report</h2>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-700">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="space-y-4 px-6 py-6">
          <div>
            <label htmlFor="reportName" className="block text-sm font-medium text-slate-900 mb-2">
              Report Name
            </label>
            <input
              id="reportName"
          ...
          <div>
            <label htmlFor="schedule" className="block text-sm font-medium text-slate-900 mb-2">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-xl bg-white shadow-xl">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <h2 className="text-lg font-semibold text-slate-900">Run Export Now</h2>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-700">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="space-y-4 px-6 py-6">
          ...
          <div>
            <label htmlFor="exportFormat" className="block text-sm font-medium text-slate-900 mb-2">
              Export Format
            </label>
            <select
              id="exportFormat"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-xl bg-white shadow-xl">
        ...
        <div className="space-y-4 px-6 py-6">
          <div>
            <label htmlFor="exportName" className="block text-sm font-medium text-slate-900 mb-2">
              Export Name
            </label>
            <input
              id="exportName"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="frequency" className="block text-sm font-medium text-slate-900 mb-2">
                Frequency
              </label>
              <select
                id="frequency"
