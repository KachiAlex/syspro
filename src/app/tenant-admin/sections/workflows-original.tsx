      ...
      <div className="rounded-3xl border border-slate-100 bg-[#111827] p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h2 className="text-xl font-semibold text-gray-900">Lifecycle workflows</h2>
              ...
              <div key={wf.id} className={`rounded-2xl border px-4 py-3 ${editingId === wf.id ? "bg-slate-50" : ""}`}>
                {editingId === wf.id ? (
                  <div className="space-y-3">
                    <input value={editName} onChange={(e) => setEditName(e.target.value)} className="bg-white rounded-lg border px-3 py-2 w-full text-black" />
                    <div className="space-y-2">
                      {(editSteps ?? []).map((s, idx) => (
                        <div key={idx} className="flex items-center gap-2">
                          <div className="w-8 text-sm text-slate-600">Step {idx + 1}</div>
                          <input value={s.title} onChange={(e) => updateEditStep(idx, "title", e.target.value)} className="bg-white rounded-lg border px-3 py-2 flex-1 text-black" />
                          <input value={s.assignee ?? ""} onChange={(e) => updateEditStep(idx, "assignee", e.target.value)} className="bg-white w-32 rounded-lg border px-2 py-2 text-black" />
                          <input type="number" value={s.daysAfter ?? 0} onChange={(e) => updateEditStep(idx, "daysAfter", Number(e.target.value))} className="bg-white w-20 rounded-lg border px-2 py-2 text-black" />
