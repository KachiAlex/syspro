          ...
          {[
            ...
            <Link
              ...
              className={`flex items-center justify-between px-3 py-2 text-sm font-medium rounded-lg transition-colors group ${
                tab.id === 'workflows' ? 'bg-blue-50 text-blue-700' : 'hover:bg-gray-100'
              }`}
            >
              <div className="flex items-center gap-3">
                <tab.icon className={`w-4 h-4 ${tab.id === 'workflows' ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-600'}`} />
                <span className={tab.id === 'workflows' ? 'text-blue-700' : 'text-gray-700 group-hover:text-gray-900'}>{tab.name}</span>
              </div>
              {tab.count && (
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                  tab.id === 'workflows' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700 group-hover:bg-gray-200'
                }`}>
                  {tab.count}
                </span>
              )}
