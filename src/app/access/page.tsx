    ...
    <div className="min-h-screen bg-[#0B1120] flex">
      ...
      <div className="flex-1 flex items-center justify-center p-[6%] lg:p-[4%]">
        <div className="w-full max-w-[380px]">
          ...
          {/* Tabs */}
          <div className="flex mb-[22px] bg-[#111827] rounded-[10px] p-[4px]">
            <button onClick={() => setTab('tenant')} className={`flex-1 py-[8px] rounded-[8px] text-[12.5px] font-semibold font-jakarta transition-all ${tab === 'tenant' ? 'bg-[#1A2438] text-[#F8FAFC] shadow-sm' : 'text-[#64748B] hover:text-[#94A3B8]'}`}>Tenant Login</button>
            <button onClick={() => setTab('admin')} className={`flex-1 py-[8px] rounded-[8px] text-[12.5px] font-semibold font-jakarta transition-all ${tab === 'admin' ? 'bg-[#1A2438] text-[#F8FAFC] shadow-sm' : 'text-[#64748B] hover:text-[#94A3B8]'}`}>Superadmin</button>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-[14px]">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-[12px] text-[#6366F1] hover:text-[#818CF8] cursor-pointer transition-colors">Forgot password?</span>
            </div>
            <button
              type="submit"
              disabled={loading}
