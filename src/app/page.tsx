  ...
  return (
    <div className="bg-[#0B1120] min-h-screen overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center pt-[120px] pb-20 px-[6%] text-center overflow-hidden">
        <PulseCanvas />
        <div className="absolute top-[35%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] rounded-full pointer-events-none" style={{ background: 'radial-gradient(circle,rgba(227,30,36,.1) 0%,transparent 68%)' }} />

        <div className="relative z-[2] max-w-[840px]">
          <div className="inline-flex items-center gap-[7px] bg-[rgba(227,30,36,.12)] border border-[rgba(227,30,36,.3)] rounded-full px-4 py-[6px] mb-[30px] text-[11.5px] font-semibold text-[#E8286E] tracking-[.06em]">
          ...
          <h1 className="font-jakarta text-[clamp(42px,7vw,78px)] font-extrabold leading-[1.07] tracking-[-.03em] mb-[22px]" style={{ background: 'linear-gradient(160deg,#F8FAFC 40%,#94A3B8 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            Your business,<br />
            <span style={{ background: 'linear-gradient(90deg,#E31E24,#F7931E)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>running itself.</span>
          </h1>
          <p className="font-jakarta text-[clamp(16px,2.2vw,20px)] font-normal text-[#94A3B8] leading-[1.65] max-w-[560px] mx-auto mb-11">
            Pisairtel ERP connects your CRM, Finance, HR, Inventory, and Projects into one living system. Every action triggers the next — automatically.
          </p>
          <div className="flex gap-[14px] justify-center flex-wrap relative z-[2]">
            <Link href="/login" className="inline-flex items-center gap-2 px-7 py-[13px] rounded-[10px] text-white font-jakarta text-sm font-bold cursor-pointer transition-all hover:-translate-y-[2px]" style={{ background: 'linear-gradient(135deg,#E31E24,#C0208A)', boxShadow: '0 6px 20px rgba(227,30,36,.4)' }}>
              See the dashboard
        ...
        <div className="relative z-[2] mt-[72px] w-full max-w-[960px]">
          <div className="rounded-2xl p-[2px]" style={{ background: 'linear-gradient(160deg,#1A2438,#111827)', border: '1px solid rgba(255,255,255,0.07)', boxShadow: '0 40px 80px rgba(0,0,0,.6),0 0 0 1px rgba(227,30,36,.1)' }}>
            <div className="bg-[#1E2A3B] rounded-t-[14px] px-4 py-[10px] flex items-center gap-2">
              ...
              <div className="flex-1 ml-3 bg-[rgba(255,255,255,.05)] rounded-md px-3 py-1 text-[11px] text-[#94A3B8]">app.pisairtel.io/dashboard</div>
            </div>
            <div className="bg-[#111827] rounded-b-[14px] p-5 grid grid-cols-[170px_1fr] gap-4 min-h-[240px]">
              <div className="bg-[#0B1120] rounded-[10px] p-[14px]">
                {['Dashboard','Finance','CRM','HR','Inventory'].map((item, i) => (
                  <div key={item} className={`px-[10px] py-[7px] rounded-[7px] mb-[3px] text-[11.5px] font-jakarta ${i === 0 ? 'bg-[rgba(227,30,36,.15)] text-[#E8286E] font-semibold' : 'text-[#94A3B8]'}`}>{item}</div>
                ))}
              </div>
              <div>
                ...
                <div className="bg-[#0B1120] rounded-[10px] p-[14px_15px]">
                  ...
                  <svg width="100%" height="52" viewBox="0 0 300 52" preserveAspectRatio="none">
                    <defs><linearGradient id="mini-grad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#E31E24" stopOpacity=".25" /><stop offset="100%" stopColor="#E31E24" stopOpacity="0" /></linearGradient></defs>
                    ...
                    <path d="M0,42 L27,36 L55,40 L82,28 L109,32 L136,22 L163,26 L191,14 L218,18 L245,8 L272,10 L300,4" stroke="#E31E24" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </div>
              </div>
            </div>
          </div>
          <div className="absolute -bottom-9 left-[20%] right-[20%] h-[72px] pointer-events-none" style={{ background: 'radial-gradient(ellipse,rgba(227,30,36,.2) 0%,transparent 70%)' }} />
        </div>
      </section>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 border-y border-[rgba(255,255,255,0.07)] px-[6%]">
        {[
          ...
          <div key={i} className="text-center py-8 px-4 border-r border-[rgba(255,255,255,0.07)] last:border-r-0">
            <div className="font-jakarta text-[15px] font-bold text-[#E8286E] mb-[6px]">{s.t}</div>
      ...
      <section className="py-24 px-[6%]">
        <div className="text-[11.5px] font-semibold tracking-[.1em] text-[#E8286E] mb-[14px] text-center">MODULES</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-[1100px] mx-auto">
          {[
            { title: 'Finance', desc: 'Invoice to payment in one automated chain. Expenses, bills, budgets, and GL — no manual posting.', color: '#E31E24' },
            ...
            { title: 'Inventory', desc: 'Every sale posts COGS to your GL. Every low-stock item drafts a PO. Your balance sheet stays current.', color: '#E8286E' },
            ...
            <div key={f.title} className="bg-[#1A2438] border border-[rgba(255,255,255,0.07)] rounded-[14px] p-7 transition-all duration-300 hover:-translate-y-1 cursor-default group" style={{ ['--hover-color' as any]: `rgba(${f.color === '#E31E24' ? '227,30,36' : f.color === '#F59E0B' ? '245,158,11' : f.color === '#10B981' ? '16,185,129' : '232,40,110'},.25)` }} onMouseEnter={(e) => (e.currentTarget.style.borderColor = `rgba(${f.color === '#E31E24' ? '227,30,36' : f.color === '#F59E0B' ? '245,158,11' : f.color === '#10B981' ? '16,185,129' : '232,40,110'},.3)`)} onMouseLeave={(e) => (e.currentTarget.style.borderColor = 'rgba(255,255,255,0.07)')}>
      ...
      <section className="py-20 px-[6%] bg-[rgba(255,255,255,.015)] border-t border-[rgba(255,255,255,0.07)]">
        <div className="max-w-[760px] mx-auto text-center">
          ...
          {[
            ...
            <div key={s.num} className="flex gap-[22px] text-left mb-4 bg-[#1A2438] border border-[rgba(255,255,255,0.07)] rounded-[14px] p-[22px_26px] items-start transition-colors hover:border-[rgba(227,30,36,.3)]">
