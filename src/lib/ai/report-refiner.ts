...
export function refineReportText(rawText: string): RefinedReport {
  const sentences = toSentences(rawText).map(fixCommonGrammar);

  const buckets: Record<keyof Omit<RefinedReport, 'title' | 'refinedText'>, string[]> = {
    activities: [],
    meetings: [],
    blockers: [],
    nextSteps: [],
  ...
  for (const sentence of sentences) {
    const section = sentenceBelongsToSection(sentence);
    if (section) {
...
export function appraiseProductivity(
  ...
  const reportCoverage = Math.round((addressedTasks.length / tasks.length) * 100);
...
export function refineReportWithAppraisal(rawText: string, tasks: Task[], appraisalOptions?: {
  ...
  const refined = refineReportText(rawText);
