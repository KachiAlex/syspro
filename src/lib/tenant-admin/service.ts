...
export class EmployeeService {
  ...
  async getAll(tenantSlug: TenantSlug, departmentId?: ResourceId): Promise<Employee[]> {
    if (departmentId) {
      return await this.sql`select * from admin_employees where tenant_slug = ${tenantSlug} and department_id = ${departmentId} order by name`;
    }
    return await this.sql`select * from admin_employees where tenant_slug = ${tenantSlug} order by name`;
  }
...
export class ApprovalFlowService {
  ...
  async getAll(tenantSlug: TenantSlug): Promise<ApprovalFlow[]> {
    return this.sql`
  ...
  async updateFlow(tenantSlug: TenantSlug, id: ResourceId, updates: Partial<{ name: string; description: string; steps: any[]; isActive: boolean }>): Promise<ApprovalFlow> {
    const now = new Date();
    const existing = await this.getById(tenantSlug, id);
    if (!existing) {
      throw new Error("Approval flow not found");
    }

    await this.sql`
      update admin_approval_flows
      set name = ${updates.name ?? existing.name},
          steps = ${updates.steps ? JSON.stringify(updates.steps) : existing.steps},
...
export class WorkflowService {
  ...
  async duplicate(tenantSlug: TenantSlug, id: ResourceId): Promise<Workflow> {
    const original = await this.getById(tenantSlug, id);
    if (!original) {
      throw new Error("Workflow not found");
    }

    const newId = randomUUID() as ResourceId;
    const now = new Date();
    ...
    await this.sql`
      insert into admin_workflows (id, tenant_slug, name, type, trigger, steps, status, is_active, description, created_at, updated_at, created_by, updated_by)
      values (${duplicated.id}, ${tenantSlug}, ${duplicated.name}, ${duplicated.type}, ${duplicated.trigger}, ${JSON.stringify(duplicated.steps)}, ${duplicated.status}, ${duplicated.isActive}, ${duplicated.description || null}, ${now}, ${now}, null, null)
    `;
...
export class IntegrationService {
  ...
  async getAll(tenantSlug: TenantSlug): Promise<Integration[]> {
    return this.sql`
  ...
  async getById(tenantSlug: TenantSlug, id: ResourceId): Promise<Integration | null> {
    const results = await this.sql`
      select * from admin_integrations
      where id = ${id} and tenant_slug = ${tenantSlug}
    `;
  ...
  async create(tenantSlug: TenantSlug, data: Partial<Integration>, createdBy?: UserId): Promise<Integration> {
    const id = randomUUID() as ResourceId;
    const now = new Date();
  ...
  async update(tenantSlug: TenantSlug, id: ResourceId, updates: Partial<Integration>): Promise<Integration | null> {
    const existing = await this.getById(tenantSlug, id);
  ...
  async delete(tenantSlug: TenantSlug, id: ResourceId): Promise<void> {
    await this.sql`
      delete from admin_integrations
      where id = ${id} and tenant_slug = ${tenantSlug}
    `;
  }
}
