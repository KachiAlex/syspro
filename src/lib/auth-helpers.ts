...
/**
 * Get user from database - queries the users table via SQL
 */
export async function getUserFromDB(userId: string, tenantSlug: string) {
  try {
    const { sql } = await import("@/lib/sql-client");
    const rows = await sql`
      SELECT u.id, u.email, u.name, u.status, t.slug as tenant_slug, r.name as role_name
      FROM users u
      JOIN tenants t ON u.tenant_id = t.id
      LEFT JOIN user_roles ur ON ur.user_id = u.id
      LEFT JOIN roles r ON r.id = ur.role_id
      WHERE u.id::text = ${userId} AND t.slug = ${tenantSlug}
      LIMIT 1
    `;
