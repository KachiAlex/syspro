...
export async function ensureBillTables(sql = SQL) {
  ...
  try {
    ...
    console.warn("Bill tables not found, creating...");
    try {
      await db.query(`
      ...
      console.warn('ensureBillTables: ignoring bills create error:', e.message);
    }
    ...
    try {
      ...
      console.warn('ensureBillTables: ignoring bill_items create error:', e.message);
    }
    ...
    for (const idxSql of indexes) {
      try {
        ...
        console.warn('ensureBillTables: ignoring index error:', e.message);
      }
    }
  }
}
